...
kill @e[tag=bullet,type=armor_stand,tag=matchedid,scores={ribbontimer=9..}]
execute as @e[tag=bullet,tag=matchedid] at @s run tp @s ^ ^ ^15
...
execute if score @s shoottimer matches 3 run stopsound @a[distance=..40] master lightjet.guns.shoot
